...
import { LeadResearchPanel } from "@/components/LeadResearchPanel";
import { getLeadEnrichmentSuggestions } from "@/lib/lead-enrichment";
import { listLeadResearchRuns, toLeadResearchRunDto } from "@/lib/research-workflow";
...
export default async function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const researchRuns = await listLeadResearchRuns(lead.id);
  const initialEnrichmentSuggestions = Object.fromEntries(await Promise.all(
    researchRuns.filter((run) => run.status === "APPROVED").map(async (run) => [
      run.id,
      await getLeadEnrichmentSuggestions(lead.id, run.id),
    ] as const),
  ));
  return <div className="page">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      <div className="space-y-5">
        ...
        <LeadResearchPanel leadId={lead.id} hasWebsite={Boolean(lead.websiteUrl)} initialEnrichmentSuggestions={initialEnrichmentSuggestions} initialRuns={researchRuns.map(toLeadResearchRunDto)} />
